...
public class SoftParkView extends JFrame {
	...
	private JButton buttonValetInvoice, buttonValetLostTicket, buttonCashierVehicleOut, buttonCashierMultiVehicleIn;
	...
	private JTextField textTicket, textCashierMultiTicket;
	...
	public SoftParkView(int stationId) {
...
//		userId = Db.getUserId(loginDialog.getUsername());
		...
		this.setTitle("Softpark Paseo - (" + user.getLogin() + ") " + user.getName());
		...
		}else if(stationInfo.getType() == 5){
			stationMode = "Multi";
			tabbedPane.addTab("Sistema de Cobro Entrada/Salida", createCashierMultiTab());
		}
	...
	private JPanel createValetTab() {
		...
		theTab.add(createCashier());
		
		return theTab;
	}
	
	private JPanel createCashierMultiTab() {
		JPanel theTab = new JPanel();
		theTab.setLayout(new GridLayout(0, 2));
		...
		JPanel panelButtonVehicle = new JPanel(new GridBagLayout());
		...
		cs.gridy = 0;
		cs.gridwidth = 6;
		panelButtonVehicle.add(labelTicket, cs);
		
		textCashierMultiTicket = new JTextField(6);
		textCashierMultiTicket.addFocusListener(lForText);
		textCashierMultiTicket.setActionCommand("multi.text.type");
		textCashierMultiTicket.addActionListener(lForText);
		cs.gridx = 0;
		cs.gridy = 1;
		cs.gridwidth = 6;
		panelButtonVehicle.add(textCashierMultiTicket, cs);
		
		buttonCashierMultiVehicleIn = new JButton("Ingresar");
		buttonCashierMultiVehicleIn.setActionCommand("multi.vehicle.in");
		buttonCashierMultiVehicleIn.addActionListener(lForButton);
		buttonCashierMultiVehicleIn.addKeyListener(lForButton);
		buttonCashierMultiVehicleIn.setPreferredSize(new Dimension(200,100));
		cs.gridx = 0;
		cs.gridy = 5;
		cs.gridwidth = 6;
		panelButtonVehicle.add(buttonCashierMultiVehicleIn, cs);
		
		thePanel.add(panelButtonVehicle, BorderLayout.CENTER);
		theTab.add(thePanel);
		theTab.add(createCashier());
	...
	private JPanel createCashierTab() {
		...
		buttonCashierVehicleOut = new JButton("Pagar");
		buttonCashierVehicleOut.setActionCommand("normal.vehicle.out");
		buttonCashierVehicleOut.addActionListener(lForButton);
		buttonCashierVehicleOut.addKeyListener(lForButton);
		buttonCashierVehicleOut.setPreferredSize(new Dimension(200,100));
		
		JPanel panelButtonVehicleOut = new JPanel(new FlowLayout());
		panelButtonVehicleOut.add(buttonCashierVehicleOut);
		thePanel.add(panelButtonVehicleOut, BorderLayout.CENTER);
	...
	private class ButtonListener implements ActionListener, KeyListener {
		...
		public void actionPerformed(ActionEvent ev) {
			if(ev.getActionCommand().equalsIgnoreCase("reload-reports")) {
				...
				if(ev.getActionCommand().equalsIgnoreCase("normal.vehicle.out") || 
						ev.getActionCommand().equalsIgnoreCase("accept") || 
						ev.getActionCommand().equalsIgnoreCase("cancel") || 
						ev.getActionCommand().equalsIgnoreCase("valet.invoice") || 
						ev.getActionCommand().equalsIgnoreCase("valet.lost") ||
						ev.getActionCommand().equalsIgnoreCase("multi.vehicle.in")) {
	...
	private class TextFieldListener implements FocusListener, ActionListener {
		...
		public void focusLost(FocusEvent e) {
			if(!buttonAccept.isVisible()) {
				textCashierMultiTicket.requestFocus();
			}
		}
		...
		public void actionPerformed(ActionEvent e) {
			...
			if(e.getActionCommand().equalsIgnoreCase("multi.text.type")) {
				int ticketNumber = Integer.valueOf(textCashierMultiTicket.getText().substring(0, 11));
	...
	private class SelectValetRun implements Runnable {
		...
		public synchronized void run() {
			...
			} else if (stationMode.equals("Normal")) {
				if(transactions.size() > 0) {
					if(actionCommand.equalsIgnoreCase("normal.vehicle.out")) {
						int transactionIndex = transactionSelected(transactions, transactionsType.get(2).getId());
						if(transactionIndex > -1) {
							transactions.remove(transactionIndex);
							buttonCashierVehicleOut.setEnabled(true);
						}
					}
				} else {
					transactions.add(transactionsType.get(2));
					buttonCashierVehicleOut.setEnabled(false);
					labelPrice.setText(String.valueOf(getSubTotal(transactions)));
					enableButtons();
			...
			} else if (stationMode.equals("Multi")) {
				if(actionCommand.equalsIgnoreCase("multi.vehicle.in")) {
					Db db = new Db();
					
					int userResponse = JOptionPane.showConfirmDialog(null, "Desea ingresar un vehiculo?", "Confirme el ingreso de un vehiculo", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
				...
				if(isPrinterConnected){
					...
					try{
						...
						if(summaryHasInvoice) {
							for(Transaction t: transactions) {
								db.insertTransaction(stationId, summaryId, ticketNumber, t.getMaxAmount(), 12, 
									t.getId(), payTypes.get(0).getId(), (shiftIsDown?0:1));
							}
						}else{
							if(summaryId > 0) {
								summaryHasInvoice = true;
	...
	public void disableButtons() {
		...
		if(stationMode.equals("Valet")) {
			...
			buttonCashierVehicleOut.setEnabled(true);
			buttonCashierVehicleOut.requestFocus();
		}
	}
